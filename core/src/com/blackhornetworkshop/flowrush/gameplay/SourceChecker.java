    ...
    public void checkAndSetActor(){
        ...
        if(doveIsOn){ // отдельно проверка голубей тем же принципом
            for(int x = 0; x < doveArray.size(); x++){
                ...
                com.blackhornetworkshop.flowrush.gameplay.TileController.setPowerOn(actor, gameScreen.game.atlas);
                tempoHexArray.clear();
                sourceArrayMain = actor.getSourceArray();
        ...
        if(gameScreen.numOfReceivers == numReceiversOn){
            ...
            for(int i=0;i<gameScreen.groupArray.size();i++){
                com.blackhornetworkshop.flowrush.gameplay.TileController.setHexbackTouchOn(gameScreen.groupArray.get(i), gameScreen.game.atlas);
            }

            timerComplete = new Timer();
            timerComplete.instance().start();
            timerComplete.schedule(new Timer.Task(){ //ждем посде сборки перед переходом на след уровень
                @Override
                public void run() {
