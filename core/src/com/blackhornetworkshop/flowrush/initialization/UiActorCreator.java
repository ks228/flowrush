...
import static com.blackhornetworkshop.flowrush.FRConstants.*;
        ...
        switch (type) {
            case 1://Иконка паузы
                ...
                pauseButton.addListener(new ClickListener() {
                    ...
                    @Override
                    public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                        if (FlowRush.getInstance().screenType != ScreenType.GAME_LVL_COMPLETE) {
            ...
            case 3://Иконка рестарта уровня
                ...
                restartButton.addListener(new ButtonScaleListener() {
                    @Override
                    public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                        if(FlowRush.getInstance().screenType== ScreenType.GAME_LVL_COMPLETE_PAUSE){
                            LevelLoader.getInstance().prevLvl();
                            FlowRush.getInstance().screenType = ScreenType.GAME;
